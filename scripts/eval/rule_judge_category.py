...
def load_grouped_data(gt_path: Path, output_paths: Dict[str, Path]) -> Tuple[Dict[str, List[Dict]], Dict[str, Dict[str, List[str]]]]:
    ...
    if gt_path.is_dir():
        gt_data = []
        for jsonl_file in sorted(gt_path.glob("*.jsonl")):
            with jsonl_file.open("r", encoding="utf-8") as f:
                for line in f:
                    if line.strip():
                        item = json.loads(line.strip())
                        category = item.get("category", jsonl_file.stem)
...
def main():
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="Ground truth path: either a JSON file or a directory containing .jsonl files to merge",
    )
    parser.add_argument(
        "--b235b-path",
        type=Path,
